...
public class SortingByPrice {
    ...
    public List<Treatment> sortingByPriceHightoLow(String searchProcedure,int maxprice) {
       ...
       String query= "select * from 19agileteam16db.treatment as treatment "
               ...
               + "where (procedures.proceduresID=\""+searchProcedure+"\" or procedures.description like '%"+searchProcedure+"%')  and treatment.AveragePayments<"+maxprice+" order by treatment.AveragePayments desc"; 
    ...
    public List<Treatment> sortingByPriceLowtoHigh(String searchProcedure,int maxprice) {
      ...
      String query= "select * from 19agileteam16db.treatment as treatment "
               + "inner join 19agileteam16db.provider as provider on treatment.providerID=provider.providerID "
               + "inner join 19agileteam16db.procedures as procedures on treatment.proceduresID=procedures.proceduresID "
               + "where (procedures.proceduresID=\""+searchProcedure+"\" or procedures.description like '%"+searchProcedure+"%')  and treatment.AveragePayments<"+maxprice+" order by treatment.AveragePayments asc";   
    ...
    public List<Treatment> sortingByPriceLowtoHighUsingName(String newProcedureName) {
       ...
       return result;
        ...
        List<Treatment> result=item.sortingByPriceHightoLow("375",30000);
        List<Treatment> result1=item.sortingByPriceLowtoHigh("375",30000);
